...
function Show(Key) {
	...
	divcontent.innerHTML = "<iframe frameborder=0 scrolling=no src='PriceEdit.aspx?Key="
			+ Key + "' width='100%' height='100%'></iframe>";
	// 嵌入页
...
function Down(e) {
	...
	th = event.clientY
			- parseInt(diveditcontent.style.top.replace(/px/, ""), 10);
	tw = event.clientX
			- parseInt(diveditcontent.style.left.replace(/px/, ""), 10);
	...
	function Move(e) {
		...
		top = top < 0 ? 0 : top;
		top = top > document.body.offsetHeight - 220 ? document.body.offsetHeight - 220
				: top;

		left = left < 0 ? 0 : left;
		left = left > document.body.offsetWidth - 630 ? document.body.offsetWidth - 630
				: left;
